...
class App extends Component {
  ...
  loginHandler = (event, authData) => {
    ...
    // chris@foxcode.io
    // tester
    const graphqlQuery = `query { login(email: "${authData.email}", password: "${authData.password}") { token userId } }`;
  ...
  signupHandler = (event, authData) => {
    ...
    const graphqlQuery = ` mutation {createUser (userInput: {email: "${email}", name: "${name}", password: "${password}"}) { _id email } } `
