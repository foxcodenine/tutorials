...
def add():
    ...
    if form.validate_on_submit():
        ...
        with open('/var/www/trendy/images/{0}'.format(form.image.data.filename), 'rb') as file:

                s3_resource.Bucket(mybucket).put_object(

                        Body = file,                        # file to upload in bits
                        Key = '{0}/{1}'.format(subfolder, image_name),  # file name to be save (& and subfolder)
                        ACL = 'public-read'                 # set to public
                )
        
        # formatting image url
        pro_image_url = 'https://{0}.s3.{1}.amazonaws.com/{2}/{3}'.format(mybucket, region, subfolder, image_name)
 
        # removing image from locally
        shutil.os.remove('/var/www/trendy/images/{form.image.data.filename}'.format(form.image.data.filename))
