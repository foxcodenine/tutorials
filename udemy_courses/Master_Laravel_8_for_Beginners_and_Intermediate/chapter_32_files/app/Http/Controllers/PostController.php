...
class PostController extends Controller
{
    ...
    public function store(StorePostRequest $request)  {  
        ...
        // Saving image file and updating the $post->image_url        
        if ($hasFile) {
            ...
            Image::create([ 'path' => $fileRelativePath , 'blog_post_id' => $post->id ]);
    ...
    public function show($id)  {
        ...
        $comments = Cache::tags(['blog-post'])->remember("blog-post-{$id}-comments", Carbon::now()->addSecond(env('CACHE_TIMEOUT')), function() use ($id) {
            return Comment::where('blog_post_id', $id)->latestComments()->with('user')->get();
        });
    ...
    {        
        ...
        if ($hasFile) {    
            ...
            $file               = $request->file('image_file');
            $fileName           = $currentPost ->id . '.' . $file->guessExtension();
            $fileRelativePath   =  $file->storeAs('thumbnails', $fileName);
            ...
            if ($image) {
                ...
                Image::create([ 'path' => $fileRelativePath , 'blog_post_id' => $currentPost ->id ]);
            }
