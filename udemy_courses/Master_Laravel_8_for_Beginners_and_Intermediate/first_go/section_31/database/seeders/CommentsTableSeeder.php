...
class CommentsTableSeeder extends Seeder
{
    ...
    public function run() {
        ...
        $comments->each(function ($item, $key) use ($newsPosts, $users) {
            ...
            if ($newsPost->created_at->timestamp > $user->created_at->timestamp) {
                
                $item->created_at =  $this->faker->dateTimeBetween($newsPost->created_at, 'now');  
            } else {
                $item->created_at =  $this->faker->dateTimeBetween($user->created_at, 'now'); 
            }
            // <- MY THING
