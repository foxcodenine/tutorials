...
class Search {
  ...
  constructor() {
    ...
    this.addSearchHtml(); // _______________________________________
  ...
  keyPressDispatcher(e) {
    if (e.key === 'Escape' && this.isOverlayOpen) this.closeOverlay(); // if (e.key === 's' && !this.isOverlayOpen) this.openOverlay();
  }
  ...
  async getResults() {
    ...
    try {
      this.resultsDiv.innerHTML = '';
      const [postsResponse, pagesResponse] = await Promise.all([fetch(`${WPVars.baseURL}/wp-json/wp/v2/posts?search=${this.searchField.value}`), fetch(`${WPVars.baseURL}/wp-json/wp/v2/pages?search=${this.searchField.value}`)]);
      const [postsData, pagesData] = await Promise.all([postsResponse.json(), pagesResponse.json()]);
