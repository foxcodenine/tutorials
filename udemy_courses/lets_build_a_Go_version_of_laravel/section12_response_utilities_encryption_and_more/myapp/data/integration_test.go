...
// run test with the command: go test . --tags integration --count=1
package data
...
var (
	// Configuration variables for the PostgreSQL test instance
	host     = "localhost"
	user     = "postgres"
	password = "secret"
...
// dummyUser is a predefined user for use in tests.
var dummyUser = User{
	FirstName: "Some",
	LastName:  "Guy",
	Email:     "me@here.com",
...
// Global variables to hold database models and connection resources
var models Models                 // Variable to hold the Models struct
var testDB *sql.DB                // Variable to hold the database connection
var resource *dockertest.Resource // Variable to manage Docker resource
var pool *dockertest.Pool         // Variable to manage Docker pool

// TestMain sets up the test environment
func TestMain(m *testing.M) {
	// Set environment variable to specify the database type
	os.Setenv("DATABASE_TYPE", "postgres")
	os.Setenv("UPPER_DB_LOG", "ERROR")

	// Create a new Docker pool to manage containers
	...
	// Docker options to run a PostgreSQL container
	...
	if err != nil {
		...
		if resource != nil {
			_ = pool.Purge(resource) // Clean up the resource in case of failure
		}
	...
	// Initialize models using the connected testDB
	models = New(testDB)
...
// createTables sets up the database schema required for testing
...
func TestToken_ExpiredToken(t *testing.T) {
	...
	token, err := models.Tokens.GenerateToken(u.ID, -time.Hour*2)
	if err != nil {
		t.Error(err)
	}
