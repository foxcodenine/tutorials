...
func (app *Config) PostLoginPage(w http.ResponseWriter, r *http.Request) {
	...
	// check password
	validPassword, err := app.Models.User.PasswordMatches(password, user)
...
func (app *Config) PostRegisterPage(w http.ResponseWriter, r *http.Request) {
	...
	url := fmt.Sprintf("http://localhost:8188/activate?email=%s", u.Email)
...
func (app *Config) ActivateAccount(w http.ResponseWriter, r *http.Request) {
	// validate url
	url := r.RequestURI
	testURL := fmt.Sprintf("http://localhost:8188%s", url)
...
func (app *Config) SubscribeToPlan(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		app.Session.Put(r.Context(), "err", " Unable to find plan.")
	...
	if err := app.Models.Plan.SubscribeUserToPlan(user, *plan); err != nil {
		...
		app.Session.Put(r.Context(), "error", "Error subscribing to plain")
		http.Redirect(w, r, "members/plans", http.StatusSeeOther)
		return
