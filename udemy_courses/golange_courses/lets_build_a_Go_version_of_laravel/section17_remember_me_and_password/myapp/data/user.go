...
// CheckForRememberToken checks if a remember token exists for a given user ID.
func (u *User) CheckForRememberToken(id int, token string) bool {
	var rememberToken RememberToken // Variable to hold the retrieved remember token
	rt := RememberToken{}           // Initialize an instance of RememberToken to access its methods

	// Get the collection (table) associated with RememberToken
	collection := upper.Collection(rt.Table())

	// Find a record that matches the user ID and remember token
	res := collection.Find(up.Cond{"user_id": id, "remember_token": token})

	// Attempt to retrieve the record into the rememberToken variable
	err := res.One(&rememberToken)

	// Return true if a record is found (err is nil), otherwise return false
	return err == nil
}
