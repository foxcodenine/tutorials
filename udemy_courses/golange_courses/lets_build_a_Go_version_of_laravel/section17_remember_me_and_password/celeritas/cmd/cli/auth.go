...
func doAuth() error {
	...
	// Step 4: Copy Handlers Files -------------------------------------

	err = copyFilefromTemplate("templates/handlers/auth-handlers.go.txt", cel.RootPath+"/handlers/auth-handlers.go")
	if err != nil {
		exitGracefully(err) // Exit if there's an error copying the auth handlers file.
	}

	// Step 5: Copy Views Files ----------------------------------------

	err = copyFilefromTemplate("templates/mailer/password-reset.html.tmpl", cel.RootPath+"/mailer/password-reset.html.tmpl")
	if err != nil {
		exitGracefully(err) // Exit if there's an error copying the views file.
	}
