...
export default function Column({ column, onRename, onAddCard, onDeleteCard, moveCard }: Props) {
  ...
  // Keep hoverIndex in sync when cards are added/removed externally (e.g. AI update)
  useEffect(() => {
    hoverIndex.current = column.cards.length;
  }, [column.cards.length]);

  // Re-sync editName when column.name changes externally while not renaming
  useEffect(() => {
  ...
  return (
    <div data-testid={`column-${column.id}`} className="flex flex-col w-72 shrink-0">
      {/* Column header */}
        ...
        <div className="mt-auto pt-1">
          {isAddingCard ? (
            ...
            <button
              ...
              className="w-full flex items-center gap-1.5 px-2 py-1.5 rounded-lg text-xs text-[#888888] hover:text-[#032147] hover:bg-white/80 transition-all duration-150 font-medium"
            >
